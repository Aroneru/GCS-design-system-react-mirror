...
const containerSpecs: [string, string, string][] = [
  ...
  [
    "Posisi",
    "Semua ukuran",
    "mx-auto — selalu terpusat di ruang yang tersedia",
  ],
  [
    "Tinggi",
    "Semua ukuran",
  ...
  return (
    ...
    >
      ...
      <FlowSection id="children" title="children">
        <Lead>
          Isi yang lebarnya dibatasi. Container tidak menyentuh tata letak
          isinya — tidak ada grid, flex, atau jarak antar-anak — jadi apa pun
          yang dimasukkan tetap mengatur dirinya sendiri. Yang dikerjakan
          Container cuma tiga: membatasi lebar (<C>max-w</C> 380px lalu 1126px),
          memusatkan (<C>mx-auto</C>), dan memberi padding kiri-kanan yang
          tumbuh bertahap (20px → 56px).
        </Lead>
        <Ruang>
      ...
      <FlowSection id="as" title="as">
        <Lead>
          ...
          , atau <C>footer</C> supaya struktur halaman tetap terbaca oleh
          pembaca layar dan mesin pencari — bukan tumpukan div tanpa makna.
        </Lead>
        <Ruang>
      ...
      <FlowSection id="class-name" title="className">
        <Lead>
          ...
          melebar penuh; dan <C>max-w-*</C> bila batasnya perlu lebih sempit
          dari bawaan.
        </Lead>
        <Ruang>
      ...
      <FlowSection id="spesifikasi" title="Angka per ambang">
        <Lead>
          ...
          <em>container query</em>. Selebihnya — warna, jarak atas-bawah, tata
      ...
      <FlowSection id="playground" title="Playground">
        <Lead>
          ...
          bawaannya — atau <C>Kustom</C> untuk mengetik lebar sendiri. Angka
          pada preview diukur langsung dari elemen yang dirender, bukan dihitung
          ulang.
        </Lead>
            ...
            <Control label="Lebar kustom">
              <div className="flex flex-wrap items-center gap-3">
                <input
                  ...
                  value={customWidth}
                  onChange={(e) =>
                    setCustomWidth(clamp(Number(e.target.value)))
                  }
      ...
      <FlowSection id="penggunaan" title="Penggunaan">
        <Lead>
          Bukan contoh tetap: snippet di bawah dirakit ulang setiap kali pilihan
          di Playground berubah — saat ini varian <C>{size}</C> (
          {activeSize.max}) pada {roomLabel}. Angka pada komentarnya diambil
          dari hasil pengukuran preview, bukan ditulis tangan.
        </Lead>
        {/* Snippet dirakit dari state playground + angka hasil pengukuran,
            jadi yang dibaca selalu kode untuk preview yang sedang terlihat. */}
        <SectionCode flush>
          {"import { Container } from '@tpl/design-kit-react'\n\n"}
          {`{/* ${activeSize.max === "tanpa batas" ? "Tanpa batas lebar" : `Batas ${activeSize.max}`} di ${roomLabel}.\n`}
      ...
      <FlowSection id="properties" title="Properties">
        <Lead>
          ...
          lalu elemen dalam yang memegang lebar dan padding. <C>className</C>{" "}
          dan atribut HTML menempel di elemen luar.
        </Lead>
